...
GIT_VERSION=`cd ${GOPATH}/src/github.com/masqchips/elementum; git describe --tags`
...
then
  ...
  cd $GOPATH
  set -e
  test go build -ldflags="-w -X github.com/masqchips/elementum/util.Version=${GIT_VERSION}" -o /var/tmp/elementum github.com/masqchips/elementum
...
then
  ...
  cd $GOPATH/src/github.com/masqchips/elementum
  test make linux-x64
